...
export default function InOutPage() {
  ...
  useEffect(() => {
    const search = async () => {
      ...
      let matched =
        products.find(
          (product) => product.barcode_number === value
        ) ?? null;

      // Fallback startsWith search
      if (!matched) {
        matched =
          products.find((product) =>
            product.barcode_number.startsWith(value)
          ) ?? null;
      }
  ...
  const handleAddProduct = async (
    ...
    if (!name.trim() || !serial.trim() || !deliveryDate) {
      toast(
        'Product name, serial number, and delivery date are required'
      );
      return;
    }
    ...
    try {
      const barcodeImage = await createBarcodeImage(
        barcodeNumber,
        name.trim(),
        serial.trim()
      );
    ...
    <div className="page-shell">
      ...
      <div className="grid two-columns">
        ...
        <div className="card">
          ...
          <div className="input-group">
            <label className="label">
              Barcode number or scanner input
            </label>
          ...
          {loadingProducts || searchingBarcode ? (
            ...
            >
              ...
              <div className="text-muted">
                Serial:{' '}
                {foundProduct.serial_number}
              </div>

              <div className="text-muted">
                Barcode:{' '}
                {foundProduct.barcode_number}
              </div>
